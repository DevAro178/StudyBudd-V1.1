...
import { formatNumber } from "./CountFormat";
  ...
  return (
    <div key={room.id} className="roomListRoom">
      <div className="roomListRoom__header">
        <Link to={`/profile/${room.host.id}`} className="roomListRoom__author">
          <div className="avatar avatar--small">
            <img src={`http://localhost:8000/${room.host.avatar}`} />
          </div>
          <span>@{room.host.username}</span>
        </Link>
        <div className="roomListRoom__actions">
          <span>{timeSince(room.created)}</span>
        </div>
      </div>
      ...
      <div className="roomListRoom__meta">
        <Link to={`/room/${room.id}`} className="roomListRoom__joined">
          <svg
            version="1.1"
          ...
          {formatNumber(room.participants.length)} Joined
        </Link>
